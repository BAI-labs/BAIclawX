...
<p align="center">
  <a href="#功能特性">功能特性</a> •
  <a href="#为什么选择-clawx">为什么选择 BAIclaw</a> •
  <a href="#快速上手">快速上手</a> •
  <a href="#系统架构">系统架构</a> •
  <a href="#开发指南">开发指南</a> •
  <a href="#参与贡献">参与贡献</a>
...
**BAIclaw** 是连接强大 AI 智能体与普通用户之间的桥梁。基于 [OpenClaw](https://github.com/OpenClaw) 构建，它将命令行式的 AI 编排转变为易用、美观的桌面体验——无需使用终端。

无论是自动化工作流、连接通讯软件，还是调度智能定时任务，BAIclaw 都能提供高效易用的图形界面，帮助你充分发挥 AI 智能体的能力。

BAIclaw 预置了面向 BANK OF AI 的模型接入配置，原生支持 Windows 平台以及多语言设置。当然，你也可以通过 **设置 → 高级 → 开发者模式** 来进行精细的高级配置。
...
构建 AI 智能体不应该需要精通命令行。BAIclaw 的设计理念很简单：**强大的技术值得拥有一个尊重用户时间的界面。**
...
| 进程管理繁琐 | 自动管理网关生命周期 |
| BANK OF AI 模型接入 | 统一的 BANK OF AI 账号配置面板 |
| 技能/插件安装复杂 | 内置技能市场与管理界面 |

### 内置 OpenClaw 核心

BAIclaw 直接基于官方 **OpenClaw** 核心构建。无需单独安装，我们将运行时嵌入应用内部，提供开箱即用的无缝体验。
...
当你使用 `@agent` 选择其他智能体时，BAIclaw 会直接切换到该智能体自己的对话上下文，而不是经过默认智能体转发。各 Agent 工作区默认彼此分离，但更强的运行时隔离仍取决于 OpenClaw 的 sandbox 配置。
...
BAIclaw 还会内置预装完整的文档处理技能（`pdf`、`xlsx`、`docx`、`pptx`），在启动时自动部署到托管技能目录（默认 `~/.openclaw/skills`），并在首次安装时默认启用。额外预装技能（`find-skills`、`self-improving-agent`、`tavily-search`、`brave-web-search`、`bocha-skill`）也会默认启用；若缺少必需的 API Key，OpenClaw 会在运行时给出配置错误提示。  
...
### 🔐 安全的 BANK OF AI 集成
可配置一个或多个 BANK OF AI 账号，凭证安全存储在系统原生密钥链中。新增账号时只需要填写 API Key；进入聊天后即可通过输入框旁的模型选择器拉取 BANK OF AI 可用模型并直接切换。
...
### Web3 / AgentWallet
在 **设置 → Web3** 中通过四步 **创建 AgentWallet** 向导导入 **TRON** 私钥、设置本地**主密码**（`@bankofai/agent-wallet` 加密存储），并在平台返回绑定地址时与 BANK OF AI API Key 账户校验一致；成功页展示示例提问。可复制地址、删除钱包。钱包数据默认在 **`~/.openclaw/agent-wallet-baiclaw`**，可通过环境变量 **`AGENT_WALLET_DIR`** 覆盖（与 CLI 一致）；新建钱包的 id 为 **`baiclaw_wallet`**，加密私钥文件为 **`secret_baiclaw_wallet.json`**。旧版本曾使用 Electron **userData** 下的 `agent-wallet` 目录，如需沿用请将其内容复制到 `~/.openclaw/agent-wallet-baiclaw`。若仅用 CLI 初始化保管库、本应用尚未写入 `kv-password` 文件：应用也会按 CLI 习惯读取 **`runtime_secrets.json`** 或环境变量 **`AGENT_WALLET_BAICLAW_PASSWORD`**；也可在 **设置 → Web3** 输入一次主密码（`POST` `/api/agent-wallets/unlock`，随后写入系统密钥链）。默认目录为 Electron 的 **`app.getPath('home')`** + **`.openclaw/agent-wallet-baiclaw`**（文件夹名为两个字母 l，勿写成 `.agent-walllet`）。若启动应用时设置了 **`AGENT_WALLET_DIR`**，命令行需指向同一路径（`agent-wallet list -d "<路径>"`）。**设置 → Web3** 会显示 **`GET` `/api/agent-wallets`** 返回的 **`storagePath`**。删除**最后一个**已配置钱包时，会删除**整个保管库目录**（与 `storagePath` 一致，例如 `~/.openclaw/agent-wallet-baiclaw`），含 `kv-password` 与 `master.json`。创建钱包成功后，向导会调用 **`POST` `/api/agent-wallets/runtime-baiclaw-password`**，在 Electron **主进程**设置 **`AGENT_WALLET_BAICLAW_PASSWORD`** 并重启 Gateway，使子进程继承（不会写入操作系统或 Shell 的全局环境变量）。每次应用启动时，若 `wallets_config.json` 中已有钱包，主进程会从已保存的保管库密码（`kv-password` / 明文文件 / `runtime_secrets.json` / 启动环境变量）**再次写入**该变量，便于 Gateway 自动启动时无需再打开向导。Host API：`GET` `/api/agent-wallets`（含 `storagePath`、`vaultUnlockRequired`、`savedPasswordMismatch`、`vaultTopologyIncomplete` 等）、`POST` `/api/agent-wallets/unlock`、`POST` `/api/agent-wallets/runtime-baiclaw-password`、`POST` `/api/agent-wallets/validate-private-key`、`POST` `/api/agent-wallets`、`DELETE` `/api/agent-wallets/:id`。
...
从 [Releases](https://github.com/ValueCell-ai/BAIclaw/releases) 页面下载适用于你平台的最新版本。
...
1. **语言与区域** – 配置你的首选语言和地区
2. **BANK OF AI 供应商** – 使用 API Key 添加一个或多个 BANK OF AI 账号，并设置默认聊天供应商
3. **技能包** – 选择适用于常见场景的预配置技能
4. **验证** – 在进入主界面前测试你的配置

> Moonshot（Kimi）说明：BAIclaw 默认保持开启 Kimi 的 web search。  
> 当配置 Moonshot 后，BAIclaw 也会将 OpenClaw 配置中的 Kimi web search 同步到中国区端点（`https://api.moonshot.cn/v1`）。

### 代理设置

BAIclaw 内置了代理设置，适用于需要通过本地代理客户端访问外网的场景，包括 Electron 本身、OpenClaw Gateway，以及 Telegram 这类频道的联网请求。
...
- 如果启用了 Telegram，BAIclaw 还会把代理同步到 OpenClaw 的 Telegram 频道配置中。
...
BAIclaw 采用 **双进程 + Host API 统一接入架构**。渲染进程只调用统一客户端抽象，协议选择与进程生命周期由 Electron 主进程统一管理：
...
我们欢迎社区的各种贡献！无论是修复 Bug、开发新功能、改进文档还是翻译——每一份贡献都让 BAIclaw 变得更好。
...
我们正在启动 BAIclaw 合作伙伴计划，寻找能够帮助我们将 BAIclaw 介绍给更多客户的合作伙伴，尤其是那些有定制化 AI 智能体或自动化需求的客户。

合作伙伴负责帮助我们连接潜在用户和项目，BAIclaw 团队则提供完整的技术支持、定制开发与集成服务。
...
<p align="center">
  <img src="https://api.star-history.com/svg?repos=ValueCell-ai/BAIclaw&type=Date" alt="Stars 历史图表" />
</p>
...
BAIclaw 基于 [MIT 许可证](LICENSE) 发布。你可以自由地使用、修改和分发本软件。
