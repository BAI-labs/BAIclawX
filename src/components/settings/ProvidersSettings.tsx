...
const inputClasses = 'h-[44px] rounded-xl font-mono text-[13px] bg-secondary border-black/10 dark:border-white/10 focus-visible:ring-2 focus-visible:ring-blue-500/50 focus-visible:border-blue-500 shadow-sm transition-all text-foreground placeholder:text-foreground/40';
...
function shouldUseBaiModelSelect(
  vendorId: ProviderType | string | null | undefined,
  models: BaiModelOption[],
): boolean {
  return vendorId === 'bai' && models.length > 0;
}
...
function ProviderCard({
  ...
  const canDiscoverBaiModels = account.vendorId === 'bai' && !!baseUrl.trim();
  const usesBaiModelSelect = shouldUseBaiModelSelect(account.vendorId, modelOptions);
  ...
  useEffect(() => {
    ...
    if (!isEditing || account.vendorId !== 'bai') {
      setStoredApiKey('');
      setModelOptions([]);
  ...
  const handleRefreshBaiModels = async () => {
    ...
    if (!effectiveApiKey || !effectiveBaseUrl) {
      setModelsError(t('aiProviders.dialog.baiModelsMissingConfig'));
      return;
    }
                ...
                <div className="space-y-1.5 pt-2">
                  ...
                  {account.vendorId === 'bai' && (
                    <p className="text-[12px] text-muted-foreground">
                      {modelsError
                        ? t('aiProviders.dialog.baiModelsFailed', { error: modelsError })
                        : (modelsLoading
                          ? t('aiProviders.dialog.baiModelsLoading')
                          : t('aiProviders.dialog.baiModelsHint'))}
                    </p>
                  )}
                </div>
              )}
              ...
              <div className="space-y-3 pt-2">
                <div className="space-y-1.5">
                  ...
                  <textarea
                    ...
                    className={isDefault
                      ...
                      : "min-h-24 w-full rounded-xl border border-black/10 dark:border-white/10 bg-secondary px-3 py-2 text-[13px] font-mono outline-none focus-visible:ring-2 focus-visible:ring-blue-500/50 focus-visible:border-blue-500 shadow-sm transition-all text-foreground placeholder:text-foreground/40"}
                  />
                  <p className="text-[12px] text-muted-foreground">
                ...
                <div className="space-y-2 pt-1">
                  ...
                  {fallbackOptions.length === 0 ? (
                    ...
                    <div className={cn("space-y-2 rounded-xl border border-black/10 dark:border-white/10 p-3 shadow-sm", isDefault ? "bg-white dark:bg-card" : "bg-secondary")}>
                ...
                <Button
                  ...
                  className={cn(
                    ...
                    isDefault
                      ...
                      : "h-[44px] bg-secondary hover:bg-black/5 dark:hover:bg-white/10 shadow-sm"
                  )}
                  disabled={
                    validating
                    || saving
                ...
                <Button
                  ...
                  className={cn(
                    ...
                    isDefault
                      ...
                      : "h-[44px] w-[44px] bg-secondary border border-black/10 dark:border-white/10 hover:bg-black/5 dark:hover:bg-white/10 shadow-sm text-muted-foreground hover:text-foreground"
                  )}
                >
                  <X className="h-4 w-4" />
                </Button>
...
function AddProviderDialog({
  ...
  const canDiscoverBaiModels = selectedType === 'bai' && !!apiKey.trim() && !!baseUrl.trim();
  const usesBaiModelSelect = shouldUseBaiModelSelect(selectedType, modelOptions);
  ...
  const handleAdd = async () => {
    ...
    try {
      ...
      const resolvedModel = selectedType === 'bai'
            ...
            <div className="space-y-6">
              ...
              <div className="space-y-6 bg-transparent p-0">
                ...
                {isOAuth && supportsApiKey && (
                  <div className="flex rounded-xl border border-black/10 dark:border-white/10 overflow-hidden text-[13px] font-medium shadow-sm bg-secondary p-1 gap-1">
                    <button
                        ...
                        <div className="relative z-10 flex flex-col items-center justify-center text-center space-y-5">
                          {oauthError ? (
                            ...
                            <div className="space-y-5 w-full">
                              ...
                              <div className="flex items-center justify-center gap-3 p-4 bg-secondary border border-black/5 dark:border-white/5 rounded-xl shadow-inner">
