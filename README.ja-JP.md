...
BAIclaw は AINFT ベースのモデル接続を前提に事前設定されており、Windows および多言語設定をネイティブにサポートしています。もちろん、**設定 → 詳細設定 → 開発者モード**から高度な設定を微調整することもできます。
...
| AINFT モデル接続 | 統合された AINFT アカウント設定パネル |
...
### 🔐 セキュアな AINFT 統合
1 つ以上の AINFT アカウントを設定でき、資格情報はシステムのネイティブキーチェーンに安全に保存されます。各アカウントで Base URL とモデル選択を上書きでき、チャットは既定の AINFT アカウントに固定されます。
...
2. **AINFT プロバイダー** – API キーで 1 つ以上の AINFT アカウントを追加し、ライブの `/models` 一覧からモデルを選び、既定のチャットプロバイダーを設定
